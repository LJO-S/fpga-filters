...
def inverse_sinc_compensation_filter(
    ...
    # f is the frequency axis for firwin2: 0..1 maps to 0..fs_out/2 (output Nyquist)
    f = np.linspace(0, 1, 1000)

    fs_out = a_fs / a_R
    fpass_normalized = a_fpass / (fs_out / 2)  # passband edge in [0,1] relative to output Nyquist

    # Convert output-Nyquist-normalised f to input-rate-normalised frequency for the CIC formula
    # f=1 (output Nyquist = fs_out/2) maps to (fs_out/2)/fs_in = fs_in/(2*fs_in*R) = 1/(2*R)
    f_input = f / (2 * a_R)
    ...
    # Do not care in the stopband becausealiasing rejection is the CIC's job and not the compensation filter's
    ...
    if a_plot_en:
        w, h = freqz(coeffs, worN=2048, fs=fs_out)
        plt.figure()
        ax1 = plt.subplot(211)
...
if __name__ == "__main__":
    ...
    INPUT_FREQ = 0.05 * (FS / M) if not INTERPOLATE else 0.3 * FS
    FPASS = 0.45 * (FS / M) if not INTERPOLATE else FS / 2 * 0.8
