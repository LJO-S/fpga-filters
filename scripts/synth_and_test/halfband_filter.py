...
from ..model.halfband_filter import Halfband_decimate, Halfband_interpolate
...
class halfband_decimate_checker:
    def __init__(self, a_halfband_object: Halfband_decimate):
        self.halfband_obj: Halfband_decimate = a_halfband_object
    ...
    def post_check_wrapper(self, a_cfg: dict, a_save_plot: bool):
        def post_check(output_path: str):
            ...
            # Fetch halfband coefficients for every stage
            halfband_list = list()
            for stage in self.halfband_obj.decimate_chain:
                halfband_list.append(stage.filter_obj.taps_prototype)
            # Plot
            save_postcheck_plot_halfband(
                a_input_data=plt_input, 
